...
namespace Nop.Plugin.WebApi.MobSocial.Controllers
{
    ...
    public class SponsorApiController : BaseMobApiController
    {
        #region Fields
        ...
        public IHttpActionResult SaveSponsorData(SponsorDataModel Model)
        {
            //here we update or reject the status of battle
            if (!ModelState.IsValid)
                return Json(new { Success = false, Message = "Invalid Data" });
        ...
        public IHttpActionResult GetSponsors([FromUri] SponsorsRequestModel Model)
        {
            ...
            return Json(new {
                ...
                BattleUrl = Url.RouteUrl("VideoBattlePage", new RouteValueDictionary()
                {
